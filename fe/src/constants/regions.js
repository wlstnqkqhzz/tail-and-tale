...
export function parseRegionValue(regionValue) {
    const value = regionValue || "";
    const matchedRegion = REGIONS.find((region) => value.startsWith(region.province));
    ...
    return {
        province: matchedRegion.province,
        district: value.replace(matchedRegion.province, "").trim(),
    };
}

export function createRegionValue(province, district) {
...
export function isCompleteRegionValue(regionValue) {
    const { province, district } = parseRegionValue(regionValue);
    const region = REGIONS.find((item) => item.province === province);

    return Boolean(region && region.districts.includes(district));
}
